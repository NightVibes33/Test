...
// Interprets an allowlisted SwiftUI subset and renders native SwiftUI controls.
// Imported source is not compiled or executed as machine code.
@MainActor
final class SwiftSourcePlaygroundModel: ObservableObject {
...
struct SwiftSourcePlaygroundView: View {
    @StateObject private var model = SwiftSourcePlaygroundModel()
        ...
        NavigationView {
            VStack(spacing: 0) {
                ...
                if model.isEditing {
                    ...
                    VStack(spacing: 10) { Image(systemName: "swift").font(.largeTitle); Text("Import or edit a .swift file, then tap Run.") }
                }
            }
...
private struct NativeView: View {
    ...
    @ObservedObject var model: SwiftSourcePlaygroundModel
    ...
    private func makeBaseView() -> AnyView {
        ...
        case .navigationStack:
            return AnyView(NavigationView { children })
        case .form:
